...
app.use('/node_modules', express.static(config.node_module_path))
app.use('/public', express.static(config.public_path))

// 配置使用 nunjucks 模板
// numjucks 模板引擎没有对模板文件名的后缀名做特定限制
// 如果文件名是 a.html 则渲染的时候就需要传递 a.html
// 如果文件名是 b.nujs 则渲染的时候就需要传递 b.nujs
// nunjucks 模板引擎默认会缓存输出过的文件
// 这里为了开发方便，所以把缓存禁用掉，从而可以实时的查看到模板文件修改的变化
